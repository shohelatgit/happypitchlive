...
import { Building2, Presentation, FileText, Briefcase, TrendingUp, ChevronRight, ArrowRight } from 'lucide-react';
...
const ActionButton = ({ variant = 'primary', children, onClick, style, isMobile = false }: {
  ...
  return <button onClick={onClick} style={{ ...baseStyle, ...variants[variant] }}>
    {children}
  </button>;
};
    ...
    <div style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', backgroundColor: COLORS.white, boxSizing: 'border-box' }}>
      ...
      <section style={{ width: '100%', padding: isMobile ? '120px 24px 80px' : '180px 120px 100px', background: `linear-gradient(to bottom, ${COLORS.lightGrayBg}, ${COLORS.white})` }}>
        <div style={{ maxWidth: '900px' }}>
          ...
          <h1 style={{ fontSize: isMobile ? '36px' : '56px', fontWeight: 700, color: COLORS.black, marginBottom: '24px', fontFamily: FONTS.inter, lineHeight: isMobile ? '42px' : '67.2px' }}>
            Presentation Services for Pharma Companies
          </h1>
          <p style={{ fontSize: isMobile ? '16px' : '20px', color: COLORS.grayText, fontFamily: FONTS.inter, maxWidth: '700px', lineHeight: isMobile ? '24px' : '30px' }}>
      ...
      <section style={{ width: '100%', padding: isMobile ? '60px 24px' : '80px 120px' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <div style={{ maxWidth: '800px', marginBottom: '48px' }}>
            <h2 style={{ fontSize: isMobile ? '28px' : '36px', fontWeight: 700, color: COLORS.black, marginBottom: '16px', fontFamily: FONTS.inter }}>
              Our Services
            </h2>
            <p style={{ fontSize: '16px', color: COLORS.grayText, fontFamily: FONTS.inter }}>
              Professional presentation and pitch deck services tailored for the pharmaceutical industry.
            </p>
          </div>
              ...
              return (
                ...
                >
                  <div style={{
                    ...
                    <p style={{ fontSize: '14px', fontWeight: 500, color: COLORS.grayText, marginBottom: '16px', fontFamily: FONTS.inter }}>
                      {service.subtitle}
                    </p>
                    <p style={{ fontSize: '16px', color: COLORS.grayText, marginBottom: '24px', fontFamily: FONTS.inter, lineHeight: '24px' }}>
                      {service.description}
                    </p>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: COLORS.black, fontWeight: 500, fontFamily: FONTS.inter }}>
      ...
      <section style={{ width: '100%', padding: isMobile ? '60px 24px' : '80px 120px', backgroundColor: COLORS.lightGrayBg }}>
        <div style={{ maxWidth: '900px', margin: '0 auto' }}>
          <h2 style={{ fontSize: isMobile ? '28px' : '36px', fontWeight: 700, color: COLORS.black, marginBottom: '24px', fontFamily: FONTS.inter }}>
            Why Pharmaceutical Companies Choose Us
          </h2>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            <div style={{ display: 'flex', gap: '16px' }}>
              <div style={{ width: '8px', height: '8px', backgroundColor: COLORS.black, borderRadius: '50%', marginTop: '8px', flexShrink: 0 }}></div>
              <p style={{ fontSize: '16px', color: COLORS.grayText, fontFamily: FONTS.inter, lineHeight: '24px' }}>
                <strong style={{ color: COLORS.black }}>Deep Industry Expertise:</strong> We understand clinical trial data, regulatory pathways, and the unique challenges of bringing therapeutics to market.
              </p>
            </div>
            <div style={{ display: 'flex', gap: '16px' }}>
              <div style={{ width: '8px', height: '8px', backgroundColor: COLORS.black, borderRadius: '50%', marginTop: '8px', flexShrink: 0 }}></div>
              <p style={{ fontSize: '16px', color: COLORS.grayText, fontFamily: FONTS.inter, lineHeight: '24px' }}>
                <strong style={{ color: COLORS.black }}>Scientific Storytelling:</strong> We translate complex mechanism-of-action data into compelling narratives that resonate with healthcare investors.
              </p>
            </div>
            <div style={{ display: 'flex', gap: '16px' }}>
              <div style={{ width: '8px', height: '8px', backgroundColor: COLORS.black, borderRadius: '50%', marginTop: '8px', flexShrink: 0 }}></div>
              <p style={{ fontSize: '16px', color: COLORS.grayText, fontFamily: FONTS.inter, lineHeight: '24px' }}>
                <strong style={{ color: COLORS.black }}>Confidentiality:</strong> We handle sensitive clinical and pre-clinical data with the utmost security and discretion.
              </p>
            </div>
          </div>
        </div>
      ...
      <section style={{ width: '100%', padding: isMobile ? '60px 24px' : '80px 120px', backgroundColor: COLORS.darkGray, textAlign: 'center' }}>
        <h2 style={{ fontSize: isMobile ? '28px' : '36px', fontWeight: 700, color: COLORS.white, marginBottom: '16px', fontFamily: FONTS.inter }}>
          Ready to elevate your pharma presentations?
        </h2>
